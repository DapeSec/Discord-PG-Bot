...
async def _send_discord_message_async(channel_id, message_content):
    ...
    try:
        ...
        print(f"🔍 DEBUG: Brian attempting to send to channel {channel_id_int}")
...
async def on_message(message):
    ...
    if message.content.lower().startswith('!brian'):
        ...
        print(f"   🎯 Brian's mention string: {BRIAN_BOT_MENTION_STRING}")
    ...
    if not should_initiate_orchestration:
        ...
        print(f"   🎯 Brian mentioned: {client.user.mentioned_in(message)}")
